...
vector<size_t> stable_matching(const vector<vector<size_t>>& preferences) {
    ...
    vector<size_t> proposals(n, -1);  // Array to store proposals, initialized to -1
    vector<size_t> matches(n, -1);     // Array to store final matches, initialized to -1
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        for (size_t k = 0; k < n; ++k) {
            if (matches[k] == -1 && preferences[k][j] < preferences[k][proposals[city]]) {
                city = k;
            }
        }
